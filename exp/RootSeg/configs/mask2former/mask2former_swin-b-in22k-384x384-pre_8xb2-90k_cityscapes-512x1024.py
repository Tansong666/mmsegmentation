...
num_classes = 19
data_root = 'J:/dataset/public/segmentation/cityscapes'
train_img_path = 'leftImg8bit/train'
train_seg_map_path = 'gtFine/train'
val_img_path = 'leftImg8bit/val'
val_seg_map_path = 'gtFine/val'

# 训练基础参数
# Swin-B pretrained on ImageNet-22K
...
model = dict(
    ...
    decode_head=dict(
        ...
        in_channels=[128, 256, 512, 1024], # Swin-B 通道数
        strides=[4, 8, 16, 32],
        feat_channels=256,
        out_channels=256,
...
# Swin Transformer 建议的自定义 Key 配置
# 设置 backbone 为 lr_mult=0.1
# 设置所有 norm 层, position_embedding, query_embedding, level_embedding 为 decay_mult=0.0
...
optim_wrapper = dict(
    type='OptimWrapper',
    optimizer=dict(
        type='AdamW', lr=0.0001, betas=(0.9, 0.999), weight_decay=0.05, eps=1e-08),
...
# 断点保存与工作目录
work_dir = './work_dirs/mask2former_swin-b-in22k-384x384-pre_8xb2-90k_cityscapes-512x1024'
