...
# 使用本地的 MSDeformAttn，避免导入 dinov3 包引起的依赖问题
from mmseg_custom.models.utils.ms_deform_attn import MSDeformAttn

from mmengine.model import BaseModule
from mmseg.registry import MODELS  # 引入 MMSeg 注册器
...
class DINOv3_Adapter(BaseModule):
    """DINOv3 Adapter for semantic segmentation.
    
    This adapter wraps a DINOv3 Vision Transformer backbone and adds
    multi-scale feature extraction capabilities using deformable attention.
    
    Args:
        backbone_cfg (dict): Configuration dict for the DINOv3 backbone.
            Should contain 'type' and other backbone-specific parameters.
        interaction_indexes (list): Indices of ViT layers to extract features from.
            Default: [9, 19, 29, 39] for ViT-g (40 layers).
        pretrain_size (int): Size of pretrained image resolution. Default: 512.
        conv_inplane (int): Number of input channels for SPM. Default: 64.
        n_points (int): Number of sampling points in deformable attention. Default: 4.
        deform_num_heads (int): Number of heads in deformable attention. Default: 16.
        drop_path_rate (float): Drop path rate. Default: 0.3.
        init_values (float): Init values for layer scale. Default: 0.0.
        with_cffn (bool): Whether to use ConvFFN. Default: True.
        cffn_ratio (float): Ratio of ConvFFN hidden dim. Default: 0.25.
        deform_ratio (float): Ratio for deformable attention. Default: 0.5.
        add_vit_feature (bool): Whether to add ViT features. Default: True.
        use_extra_extractor (bool): Whether to use extra extractors. Default: True.
        with_cp (bool): Whether to use checkpoint. Default: True.
    ...
    def __init__(
        ...
        super(DINOv3_Adapter, self).__init__(init_cfg=init_cfg)
